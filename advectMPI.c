...
int main(int argc, char **argv)
{
	...
	MPI_Sendrecv(*(matrix+1), mysize, MPI_DOUBLE, source, 0, *matrix, mysize, MPI_DOUBLE, dest, 0,comm2d, &stat);
	MPI_Sendrecv(*(matrix+(mysize-2)), mysize, MPI_DOUBLE, dest, 0, *(matrix+(mysize-1)), mysize, MPI_DOUBLE, source, 0,comm2d, &stat);
	MPI_Barrier(MPI_COMM_WORLD);
	MPI_Cart_shift(comm2d, 1, 1, &source, &dest);
	MPI_Sendrecv(*(matrix+1)+1, 1, ghost_type, source, 0, *(matrix+1), 1, ghost_type, dest, 0,comm2d, &stat);
	MPI_Sendrecv(*(matrix+1)+mysize-2, 1, ghost_type, source, 0, *(matrix+1) + (mysize-1), 1, ghost_type, dest, 0,comm2d, &stat);
	MPI_Barrier(MPI_COMM_WORLD);
	/************Cart Shift ----  Sendrecv row and ghost**********/
	/****************** END INITIAL FILL**************/
	...
	for(k = 0; k < NT; k++)
	{
		...
		MPI_Barrier(MPI_COMM_WORLD);/*make sure everyone has done this*/

		/************** BEGIN ****************/
		/************Cart Shift ----  Sendrecv row and ghost**********/
		MPI_Cart_shift(comm2d, 0, 1, &source, &dest);
		MPI_Sendrecv(*(matrix1+1), mysize, MPI_DOUBLE, source, 0, *matrix1, mysize, MPI_DOUBLE, dest, 0,comm2d, &stat);
		MPI_Sendrecv(*(matrix1+(mysize-2)), mysize, MPI_DOUBLE, dest, 0, *(matrix1+(mysize-1)), mysize, MPI_DOUBLE, source, 0,comm2d, &stat);
		MPI_Barrier(MPI_COMM_WORLD);
		MPI_Cart_shift(comm2d, 1, 1, &source, &dest);
		MPI_Sendrecv(*(matrix1+1)+1, 1, ghost_type, source, 0, *(matrix1+1), 1, ghost_type, dest, 0,comm2d, &stat);
		MPI_Sendrecv(*(matrix1+1)+mysize-2, 1, ghost_type, source, 0, *(matrix1+1) + (mysize-1), 1, ghost_type, dest, 0,comm2d, &stat);
		MPI_Barrier(MPI_COMM_WORLD);
		/************Cart Shift ----  Sendrecv row and ghost**********/
		/****************** END **************/
